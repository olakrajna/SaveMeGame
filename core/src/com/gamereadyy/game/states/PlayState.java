...
public class PlayState extends State {
    ...
    public void update(float dt) {
        ...
        for (int i = 0; i < tubes.size; i++) {
            ...
            /**jeśli żółwik nakłada się z butelką, gracz zyskuje punkt, a butelka znika*/
            if(tube.collect(turtle.getBounds()) && rep ) {
                tube.getPosBottle().y = tube.getPosBottle().y + 1000;
                score++;
                rep = false;
            }
    ...
    public void render(SpriteBatch sb) {
        ...
        if (state == 1) {
            ...
            for(Tube tube: tubes) {
                ...
                sb.draw(tube.getBottle(), tube.getPosBottle().x, tube.getPosBottle().y);
            }
        ...
        } else if(state==2) /**jeśli gra jest przegrana*/
        {
            ...
            font1.draw(sb, "Your collected plastic bottles:  " + String.valueOf(score),cam.position.x - (cam.viewportWidth/2) + 30, 650 );
            font1.draw(sb, "This is about " + String.valueOf(score*0.025) + " kg of plastic.",cam.position.x - (cam.viewportWidth/2) + 30, 600 );
            font1.draw(sb, "And that's only " + String.valueOf((score*0.025f/80000000)) + "% of\nwhat into the oceans each year. ",cam.position.x - (cam.viewportWidth/2) + 30, 550 );
